...
import tailDomains from '../content/v2/tail-domains.mjs';
...
uniq(domains.map(d=>d.id),'domain');
...
for(const d of domains){
  ...
  for(const m of d.modules){
    ...
    for(const p of m.prerequisites||[])if(!moduleIds.has(p)&&!unitIds.has(p))warnings.push(`Unresolved prerequisite ${p} on module ${m.id}`);
...
const unmappedLegacy=v1.map(t=>t.id).filter(id=>!mapped.has(id));
if(unmappedLegacy.length)warnings.push(`V1 topics not explicitly migrated: ${unmappedLegacy.join(', ')}`);
...
const catalog={generatedAt:new Date().toISOString(),domains,resources,stats,unmappedLegacy};
